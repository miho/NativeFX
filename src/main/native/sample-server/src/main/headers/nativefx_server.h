...
typedef redraw_callback std::function<void (uchar* buffer_data, int W, int H)>
typedef event_callback  std::function<void (event* evt)>
...
void start(int argc, char *argv[], redraw_callback redraw, event_callback events)
{
    ...
    while(true) {
        ...
        redraw_callback(buffer_data, W, H);
